/* Gold Breeze Background Animation — módulo ES
 * Lógica original sin cambios. Solo envuelto en init() + export.
 * Guard: si #particle-canvas no existe, sale limpio.
 */
const GoldBreeze = {
  init() {
    ...
    if (!PC) return; // guard: no hero, no canvas
    ...
    class Mote {
      ...
      update() {
        ...
        if (this.x > W + 10) { this.reset(false); this.x = -5; }
      }
      draw() {
        pCtx.save();
