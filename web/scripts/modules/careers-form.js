/* CareersForm — módulo ES
 * Lógica original de send_careers_form.js sin cambios. Solo envuelto en init() + export.
 * Selectores reales: #careers_form, #careers_form_msg, .careers_submit,
 *                    #cv_file, #cv_trigger, #file_name, .form_input, .form_label
 * Mantiene CSRF token, validación en tiempo real y preview del archivo.
 */
const CareersForm = {
  init() {
    ...
    function validateField(field, value) {
      switch (field) {
        ...
        case 'phone':       return /^[\d\s\+\-\(\)]{7,}$/.test(value.trim());
        case 'position':    return value.trim().length > 0;
        case 'experience':  return value.trim().length === 0 || value.trim().length >= 5;
    ...
    // Validación en tiempo real + labels
    ...
    // Trigger y preview del archivo CV
    ...
    form.addEventListener('submit', async (e) => {
      ...
      if (!csrfToken) {
        ...
        if (submitBtn) { submitBtn.disabled = false; submitBtn.value = 'Enviar Aplicación'; }
        return;
      }
