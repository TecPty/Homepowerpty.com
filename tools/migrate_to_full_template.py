"""
Migra todas las páginas con plantilla minimal a la plantilla full luxury.
Lee contenido de cada página (nombre, SKU, specs, WA link, imágenes)
y genera el HTML completo con announcement bar, lightbox, gallery-strip.
"""
import re
import os
from pathlib import Path
from PIL import Image

BASE = Path(r'c:\Users\HP 15\Homepowerpty.com')

MINIMAL_PAGES = [
    r'productos\abanicos\hp-008',
    r'productos\asador\hp-014',
    r'productos\basculas\hp-023',
    r'productos\batidoras\hp-021',
    r'productos\calderos\hp-025',
    r'productos\calderos\hp-026',
    r'productos\calderos\hp-027',
    r'productos\calderos\hp-028',
    r'productos\calderos\hp-029',
    r'productos\calderos\hp-030',
    r'productos\calderos\hp-031',
    r'productos\calderos\hp-032',
    r'productos\calderos\hp-033',
    r'productos\calderos\hp-034',
    r'productos\extensiones\hp-050',
    r'productos\extensiones\hp-051',
    r'productos\extensiones\hp-052',
    r'productos\extensiones\hp-053',
    r'productos\extensiones\hp-054',
    r'productos\extensiones\hp-055',
    r'productos\extensiones\hp-056',
    r'productos\extensiones\hp-057',
    r'productos\extensiones\hp-058',
    r'productos\extensiones\hp-059',
    r'productos\extensiones\hp-060',
    r'productos\extensiones\hp-061',
    r'productos\extensiones\hp-062',
    r'productos\extensiones\hp-063',
    r'productos\extensiones\hp-064',
    r'productos\extensiones\hp-065',
    r'productos\extensiones\hp-066',
    r'productos\extensiones\hp-067',
    r'productos\extensiones\hp-068',
    r'productos\licuadoras\hp-020',
    r'productos\ollas\hp-035',
    r'productos\ollas\hp-036',
    r'productos\ollas\hp-037',
    r'productos\ollas\hp-038',
    r'productos\ollas\hp-039',
    r'productos\regletas\hp-070',
    r'productos\regletas\hp-071',
    r'productos\regletas\hp-072',
    r'productos\soportes-tv\hp-040',
    r'productos\soportes-tv\hp-041',
    r'productos\soportes-tv\hp-042',
    r'productos\soportes-tv\hp-043',
    r'productos\teteras\hp-013',
]

CATEGORY_LABELS = {
    'calderos': 'Calderos',
    'extensiones': 'Extensiones',
    'licuadoras': 'Licuadoras',
    'ollas': 'Ollas a Presión',
    'regletas': 'Regletas',
    'soportes-tv': 'Soportes TV',
    'abanicos': 'Abanicos',
    'asador': 'Asadores',
    'basculas': 'Básculas',
    'batidoras': 'Batidoras',
    'teteras': 'Teteras',
}

IMG_ALTS = {
    'PRODUCTO_PRINCIPAL': 'Vista principal',
    'LIFESTYLE_DARK': 'Lifestyle oscuro',
    'LIFESTYLE_FAMILY': 'Lifestyle familia',
    'LIFESTYLE_KITCHEN': 'Lifestyle cocina',
    'LIFESTYLE_FOREST': 'Lifestyle bosque',
    'LIFESTYLE_SUNSET': 'Lifestyle atardecer',
    'LIFESTYLE_BOX': 'Lifestyle caja',
    'PRODUCTO_CAJA': 'Caja del producto',
    'PRODUCTO_CAJA_1': 'Caja detalle',
    'DETAIL_LOGO': 'Detalle logo',
    'BRAND_COMMERCIAL': 'Imagen comercial',
    'PRODUCTO_FRONTAL': 'Vista frontal',
    'PRODUCT_BACK': 'Vista posterior',
    'PRODUCTO_BACK': 'Vista posterior',
    'LIFESTYLE_WHITE': 'Lifestyle blanco',
    'LIFESTYLE_GREY': 'Lifestyle gris',
}

def get_alt(filename):
    stem = Path(filename).stem
    return IMG_ALTS.get(stem, stem.replace('_', ' ').title())

def convert_png_if_needed(img_dir):
    """Convierte PNGs a WebP si no existe versión WebP."""
    converted = []
    for png in img_dir.glob('*.png'):
        webp = png.with_suffix('.webp')
        if not webp.exists():
            try:
                img = Image.open(png)
                img = img.convert('RGBA') if img.mode in ('RGBA', 'LA', 'P') else img.convert('RGB')
                img.save(webp, 'WEBP', quality=88, method=6)
                converted.append(png.name)
            except Exception as e:
                print(f'  [WARN] No se pudo convertir {png.name}: {e}')
    return converted

def get_images(img_dir):
    """Devuelve lista de WebP ordenada: PRODUCTO_PRINCIPAL primero."""
    webps = sorted([f.name for f in img_dir.glob('*.webp')])
    # Poner PRODUCTO_PRINCIPAL al inicio
    principal = [f for f in webps if 'PRODUCTO_PRINCIPAL' in f]
    others = [f for f in webps if 'PRODUCTO_PRINCIPAL' not in f]
    return principal + others

def extract_content(html):
    """Extrae nombre, SKU, descripción, spec pills y link WA."""
    # Product name
    m = re.search(r'<h1[^>]*class="pdp-name"[^>]*>(.*?)</h1>', html, re.DOTALL)
    product_name = m.group(1).strip() if m else ''

    # SKU
    m = re.search(r'<span[^>]*class="pdp-sku"[^>]*>MOD:\s*(.*?)</span>', html, re.DOTALL)
    sku = m.group(1).strip() if m else ''

    # Meta description
    m = re.search(r'<meta name="description" content="(.*?)"', html)
    description = m.group(1).strip() if m else f'{product_name} — Distribuidor B2B exclusivo en Panamá.'

    # Spec pills (li format)
    m = re.search(r'<ul[^>]*class="pdp-spec-pills"[^>]*>(.*?)</ul>', html, re.DOTALL)
    specs = []
    if m:
        specs = re.findall(r'<li>(.*?)</li>', m.group(1))

    # WhatsApp link
    m = re.search(r'href="(https://wa\.me/[^"]+)"', html)
    wa_link = m.group(1) if m else f'https://wa.me/50769838322'

    return product_name, sku, description, specs, wa_link

OVERLAY_SVG = ('<svg width="22" height="22" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">'
               '<circle cx="11" cy="11" r="8"/>'
               '<line x1="21" y1="21" x2="16.65" y2="16.65"/>'
               '<line x1="11" y1="8" x2="11" y2="14"/>'
               '<line x1="8" y1="11" x2="14" y2="11"/>'
               '</svg>')

WA_SVG = ('<svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">'
          '<path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966'
          '-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059'
          '-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025'
          '-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074'
          '-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487'
          '.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248'
          '-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/>'
          '<path d="M12 0C5.373 0 0 5.373 0 12c0 2.123.555 4.116 1.527 5.842L0 24l6.335-1.652A11.954 11.954 0 0 0'
          ' 12 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 21.818a9.818 9.818 0 0 1-5.013-1.376l-.36-.214-3.727'
          '.977.992-3.626-.235-.373A9.818 9.818 0 1 1 12 21.818z"/>'
          '</svg>')

def build_thumbs(images):
    lines = []
    for i, img in enumerate(images):
        alt = get_alt(img)
        active = ' active' if i == 0 else ''
        lines.append(
            f'                        <button class="pdp-thumb{active}"\n'
            f'                                onclick="pdpSetImage(\'./img/{img}\', this)"\n'
            f'                                aria-label="Ver {alt.lower()}" role="listitem">\n'
            f'                            <img src="./img/{img}" alt="{alt}">\n'
            f'                        </button>'
        )
    return '\n'.join(lines)

def build_strip(images):
    lines = []
    for i, img in enumerate(images):
        alt = get_alt(img)
        lines.append(
            f'                        <div class="pdp-strip-item" onclick="pdpOpenLightboxAt({i})" '
            f'role="button" tabindex="0" aria-label="Ampliar {alt}">\n'
            f'                            <img src="./img/{img}" alt="{alt}" loading="lazy">\n'
            f'                            <div class="pdp-strip-overlay" aria-hidden="true">{OVERLAY_SVG}</div>\n'
            f'                        </div>'
        )
    return '\n'.join(lines)

def build_spec_pills(specs):
    if not specs:
        return ''
    lines = []
    for spec in specs:
        lines.append(f'                        <div class="pdp-spec-pill"><span>{spec}</span></div>')
    return '\n'.join(lines)

def build_spec_table(sku, specs):
    rows = [f'<tr><td>Modelo</td><td>{sku}</td></tr>']
    for spec in specs:
        rows.append(f'                                            <tr><td colspan="2">{spec}</td></tr>')
    return '\n'.join(rows)

def build_pdp_images_js(images):
    items = [f"'./img/{img}'" for img in images]
    return ',\n            '.join(items)

def generate_html(product_name, sku, description, specs, wa_link, category, images):
    cat_label = CATEGORY_LABELS.get(category, category.title())
    first_img = images[0] if images else 'PRODUCTO_PRINCIPAL.webp'

    thumbs = build_thumbs(images)
    strip = build_strip(images)
    spec_pills = build_spec_pills(specs)
    spec_table = build_spec_table(sku, specs)
    pdp_images_js = build_pdp_images_js(images)

    return f'''<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600&family=Playfair+Display:wght@400;600;700&display=swap" rel="stylesheet">
    <link rel="icon" href="../../../media/icons/favicon/favicon-32x32.png" type="image/png">
    <title>{product_name} — HomePower PTY</title>
    <meta name="description" content="{description}">
    <meta name="robots" content="index, follow">
    <link rel="stylesheet" href="../../../styles/luxury.css">
    <link rel="stylesheet" href="../../../styles/product-v2.css">
</head>
<body class="product-page">

    <!-- ANNOUNCEMENT BAR -->
    <div class="announcement-bar">
        <div class="container text-center">
            <span>Stock exclusivo · Garantía 1 año · WhatsApp: (507) 6983-8322</span>
        </div>
    </div>

    <!-- NAVBAR -->
    <header class="luxury-header">
        <div class="container header-container">
            <div class="logo-container">
                <a href="../../../index.html">
                    <img src="../../../media/icons/logo/logo.png" alt="Home Power PTY" class="luxury-logo">
                </a>
            </div>
            <nav class="luxury-nav">
                <ul class="nav-links">
                    <li><a href="../../../index.html">Inicio</a></li>
                    <li><a href="../../../index.html#productos">Electrodomésticos</a></li>
                    <li><a href="../../../index.html#nosotros">Nosotros</a></li>
                    <li><a href="../../../index.html#clientes">Clientes</a></li>
                    <li><a href="../../../index.html#contacto">Contacto</a></li>
                </ul>
            </nav>
            <div class="header-actions">
                <a href="https://wa.me/50769838322" class="btn-outline-gold" target="_blank" rel="noopener">Consultar</a>
                <button class="hamburger-btn" id="menuBurger" aria-label="Abrir menú" aria-expanded="false">
                    <span></span>
                    <span></span>
                    <span></span>
                </button>
            </div>
        </div>
    </header>

    <!-- FULLSCREEN MOBILE MENU -->
    <div class="fullscreen-menu" id="fullscreenMenu" role="dialog" aria-modal="true" aria-label="Menú de navegación">
        <button class="fullscreen-menu__close" id="menuClose" aria-label="Cerrar menú">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
        </button>
        <nav>
            <ul class="fullscreen-menu__links">
                <li><a href="../../../index.html" class="fullscreen-menu-link">Inicio</a></li>
                <li><a href="../../../index.html#productos" class="fullscreen-menu-link">Electrodomésticos</a></li>
                <li><a href="../../../index.html#nosotros" class="fullscreen-menu-link">Nosotros</a></li>
                <li><a href="../../../index.html#clientes" class="fullscreen-menu-link">Clientes</a></li>
                <li><a href="../../../index.html#contacto" class="fullscreen-menu-link">Contacto</a></li>
            </ul>
        </nav>
        <a href="https://wa.me/50769838322" class="fullscreen-menu__cta" target="_blank" rel="noopener">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
            Consultar
        </a>
    </div>

    <!-- BREADCRUMB -->
    <div class="pdp-breadcrumb">
        <div class="container">
            <nav aria-label="Breadcrumb">
                <a href="../../../index.html">Inicio</a>
                <span class="crumb-sep" aria-hidden="true">›</span>
                <a href="../../../index.html#productos">Electrodomésticos</a>
                <span class="crumb-sep" aria-hidden="true">›</span>
                <a href="../../../index.html#productos">{cat_label}</a>
                <span class="crumb-sep" aria-hidden="true">›</span>
                <span class="crumb-current">{product_name}</span>
            </nav>
        </div>
    </div>

    <!-- MAIN PRODUCT -->
    <main>
        <section class="pdp-main">
            <div class="pdp-grid">

                <!-- LEFT: GALLERY -->
                <div class="pdp-gallery">
                    <div class="pdp-main-img-wrap"
                         id="pdpMainImgWrap"
                         onclick="pdpOpenLightbox()"
                         role="button"
                         tabindex="0"
                         aria-label="Ampliar imagen">
                        <img id="pdpMainImg"
                             src="./img/{first_img}"
                             alt="{product_name} — HomePower PTY">
                        <div class="pdp-zoom-hint" aria-hidden="true">
                            <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><line x1="11" y1="8" x2="11" y2="14"/><line x1="8" y1="11" x2="14" y2="11"/></svg>
                            Ampliar
                        </div>
                    </div>

                    <div class="pdp-thumbs" role="list" aria-label="Imágenes del producto">
{thumbs}
                    </div>
                </div>

                <!-- RIGHT: INFO PANEL -->
                <div class="pdp-info">
                    <span class="pdp-badge">PREMIUM</span>
                    <p class="pdp-brand">HomePower PTY</p>
                    <h1 class="pdp-name">{product_name}</h1>
                    <p class="pdp-model">{sku}</p>

                    <div class="pdp-divider" aria-hidden="true"></div>

                    <p class="pdp-short-desc">{description}</p>

                    <div class="pdp-spec-pills">
{spec_pills}
                    </div>

                    <div class="pdp-cta-group">
                        <a href="{wa_link}"
                           class="pdp-btn-primary"
                           target="_blank"
                           rel="noopener">
                            {WA_SVG}
                            Solicitar cotización B2B
                        </a>
                        <a href="../../../index.html#productos" class="pdp-btn-secondary">
                            <svg width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
                            Ver más productos
                        </a>
                    </div>

                    <!-- ACCORDION -->
                    <div class="pdp-accordion">
                        <div class="pdp-accordion-item">
                            <button class="pdp-accordion-trigger open"
                                    onclick="pdpToggle(this)"
                                    aria-expanded="true"
                                    aria-controls="acc-specs">
                                Especificaciones técnicas
                                <svg class="pdp-accordion-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                            </button>
                            <div class="pdp-accordion-body open" id="acc-specs" role="region">
                                <div class="pdp-accordion-content">
                                    <table class="pdp-specs-table">
                                        <tbody>
                                            {spec_table}
                                        </tbody>
                                    </table>
                                </div>
                            </div>
                        </div>

                        <div class="pdp-accordion-item">
                            <button class="pdp-accordion-trigger"
                                    onclick="pdpToggle(this)"
                                    aria-expanded="false"
                                    aria-controls="acc-shipping">
                                Distribución y envío
                                <svg class="pdp-accordion-icon" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24" aria-hidden="true"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
                            </button>
                            <div class="pdp-accordion-body" id="acc-shipping" role="region">
                                <div class="pdp-accordion-content">
                                    <div class="pdp-shipping-list">
                                        <div class="pdp-shipping-item">
                                            <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
                                            <div>
                                                <strong>Entrega en 2 días hábiles</strong>
                                                Logística directa para distribuidores en toda la República de Panamá.
                                            </div>
                                        </div>
                                        <div class="pdp-shipping-item">
                                            <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/></svg>
                                            <div>
                                                <strong>Factura disponible</strong>
                                                Documentación formal para empresas, cadenas y distribuidores.
                                            </div>
                                        </div>
                                        <div class="pdp-shipping-item">
                                            <svg width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>
                                            <div>
                                                <strong>Stock exclusivo</strong>
                                                Importado directo. No disponible en retail masivo.
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                </div>
            </div>
        </section>

        <!-- BELOW FOLD -->
        <div class="pdp-below">

            <section class="pdp-gallery-strip">
                <div class="container">
                    <p class="section-eyebrow pdp-gallery-eyebrow">GALERÍA COMPLETA</p>
                    <div class="pdp-gallery-strip-grid">
{strip}
                    </div>
                </div>
            </section>

            <section class="pdp-trust-bar">
                <div class="container">
                    <div class="pdp-trust-row">
                        <div class="pdp-trust-cell">
                            <div class="pdp-trust-cell-icon">
                                <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
                            </div>
                            <div>
                                <h4>2 días de entrega</h4>
                                <p>Logística directa a distribuidores en Panamá</p>
                            </div>
                        </div>
                        <div class="pdp-trust-cell">
                            <div class="pdp-trust-cell-icon">
                                <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/></svg>
                            </div>
                            <div>
                                <h4>Factura disponible</h4>
                                <p>Documentación para empresas y cadenas</p>
                            </div>
                        </div>
                        <div class="pdp-trust-cell">
                            <div class="pdp-trust-cell-icon">
                                <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><polyline points="9 12 11 14 15 10"/></svg>
                            </div>
                            <div>
                                <h4>Importado directo</h4>
                                <p>Stock exclusivo — no disponible en retail masivo</p>
                            </div>
                        </div>
                        <div class="pdp-trust-cell">
                            <div class="pdp-trust-cell-icon">
                                <svg width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
                            </div>
                            <div>
                                <h4>Soporte 24/7</h4>
                                <p>WhatsApp — respuesta en menos de 30 min</p>
                            </div>
                        </div>
                    </div>
                </div>
            </section>

            <section class="pdp-cta-band">
                <div class="container">
                    <h2>¿Listo para incluir este producto<br>en su catálogo?</h2>
                    <p>Cotización B2B personalizada para distribuidores. Respuesta en menos de 10 minutos.</p>
                    <a href="{wa_link}"
                       class="pdp-btn-primary pdp-btn-cta-centered"
                       target="_blank"
                       rel="noopener">
                        {WA_SVG}
                        Consultar precio B2B
                    </a>
                    <p class="pdp-cta-note">Sin compromiso · Toda Panamá · Stock disponible</p>
                </div>
            </section>

        </div>
    </main>

    <!-- FOOTER -->
    <footer class="luxury-footer">
        <div class="container footer-container">
            <div class="footer-col" style="align-items:flex-start;">
                <img src="../../../media/icons/logo/logo_white_premium.png" alt="Home Power PTY" class="luxury-logo" style="margin-bottom:20px;">
                <p>(507) 6983-8322</p>
                <p>ventas@homepowerpty.com</p>
            </div>
            <div class="footer-col" style="flex-direction:column; align-items:flex-start; gap:10px;">
                <a href="../../../index.html">Inicio</a>
                <a href="../../../index.html#productos">Electrodomésticos</a>
                <a href="../../../index.html#clientes">Clientes</a>
                <a href="../../../index.html#contacto">Contacto</a>
            </div>
            <div class="footer-col" style="align-items:flex-end;">
                <p style="font-size:0.8rem; color:gray;">© 2026 HomePower PTY.<br>Todos los derechos reservados.</p>
            </div>
        </div>
    </footer>

    <!-- LIGHTBOX -->
    <div class="pdp-lightbox" id="pdpLightbox" role="dialog" aria-modal="true" aria-label="Visor de imágenes">
        <button class="pdp-lightbox-close" onclick="pdpCloseLightbox()" aria-label="Cerrar visor">&#x2715;</button>
        <button class="pdp-lightbox-prev" onclick="pdpLightboxMove(-1)" aria-label="Imagen anterior">&#8592;</button>
        <img class="pdp-lightbox-img" id="pdpLightboxImg" src="./img/{first_img}" alt="Imagen ampliada del producto">
        <button class="pdp-lightbox-next" onclick="pdpLightboxMove(1)" aria-label="Imagen siguiente">&#8594;</button>
    </div>

    <script>
        const pdpImages = [
            {pdp_images_js},
        ];
        let pdpLightboxIndex = 0;
        let currentLightboxSrc = pdpImages[0] || '';

        function pdpSetImage(src, thumbEl) {{
            const mainImg = document.getElementById('pdpMainImg');
            mainImg.style.opacity = '0';
            setTimeout(() => {{ mainImg.src = src; mainImg.style.opacity = '1'; }}, 150);
            document.querySelectorAll('.pdp-thumb').forEach(t => t.classList.remove('active'));
            thumbEl.classList.add('active');
            currentLightboxSrc = src;
        }}

        function pdpOpenLightbox() {{
            const idx = pdpImages.indexOf(currentLightboxSrc);
            pdpLightboxIndex = idx >= 0 ? idx : 0;
            document.getElementById('pdpLightboxImg').src = pdpImages[pdpLightboxIndex];
            document.getElementById('pdpLightbox').classList.add('active');
            document.body.style.overflow = 'hidden';
        }}

        function pdpOpenLightboxAt(idx) {{
            pdpLightboxIndex = idx;
            document.getElementById('pdpLightboxImg').src = pdpImages[pdpLightboxIndex];
            document.getElementById('pdpLightbox').classList.add('active');
            document.body.style.overflow = 'hidden';
        }}

        function pdpCloseLightbox() {{
            document.getElementById('pdpLightbox').classList.remove('active');
            document.body.style.overflow = '';
        }}

        function pdpLightboxMove(dir) {{
            pdpLightboxIndex = (pdpLightboxIndex + dir + pdpImages.length) % pdpImages.length;
            document.getElementById('pdpLightboxImg').src = pdpImages[pdpLightboxIndex];
        }}

        function pdpToggle(btn) {{
            const body = btn.nextElementSibling;
            const isOpen = body.classList.contains('open');
            body.classList.toggle('open', !isOpen);
            btn.classList.toggle('open', !isOpen);
            btn.setAttribute('aria-expanded', String(!isOpen));
        }}

        document.addEventListener('keydown', e => {{
            if (!document.getElementById('pdpLightbox').classList.contains('active')) return;
            if (e.key === 'Escape') pdpCloseLightbox();
            if (e.key === 'ArrowRight') pdpLightboxMove(1);
            if (e.key === 'ArrowLeft') pdpLightboxMove(-1);
        }});

        // Hamburger menu
        const burger = document.getElementById('menuBurger');
        const menu   = document.getElementById('fullscreenMenu');
        const close  = document.getElementById('menuClose');
        if (burger && menu && close) {{
            burger.addEventListener('click', () => {{
                menu.classList.add('open');
                burger.setAttribute('aria-expanded', 'true');
                document.body.style.overflow = 'hidden';
            }});
            close.addEventListener('click', () => {{
                menu.classList.remove('open');
                burger.setAttribute('aria-expanded', 'false');
                document.body.style.overflow = '';
            }});
            menu.querySelectorAll('.fullscreen-menu-link').forEach(link => {{
                link.addEventListener('click', () => {{
                    menu.classList.remove('open');
                    burger.setAttribute('aria-expanded', 'false');
                    document.body.style.overflow = '';
                }});
            }});
        }}
    </script>
</body>
</html>'''


def main():
    print(f'Migrando {len(MINIMAL_PAGES)} páginas...\n')
    ok = 0
    errors = []

    for rel_path in MINIMAL_PAGES:
        product_dir = BASE / rel_path
        html_file = product_dir / 'index.html'
        img_dir = product_dir / 'img'

        if not html_file.exists():
            print(f'  [SKIP] {rel_path} — no existe index.html')
            continue

        # Convertir PNGs pendientes
        converted = convert_png_if_needed(img_dir)
        if converted:
            print(f'  [CONV] {rel_path}: {", ".join(converted)}')

        # Leer HTML actual
        html = html_file.read_text(encoding='utf-8')

        # Extraer contenido
        product_name, sku, description, specs, wa_link = extract_content(html)
        if not product_name:
            print(f'  [WARN] {rel_path} — no se pudo extraer nombre, saltando')
            errors.append(rel_path)
            continue

        # Obtener imágenes
        images = get_images(img_dir)
        if not images:
            print(f'  [WARN] {rel_path} — sin imágenes WebP')
            images = ['PRODUCTO_PRINCIPAL.webp']

        # Detectar categoría (carpeta padre)
        category = product_dir.parent.name

        # Generar HTML
        try:
            new_html = generate_html(product_name, sku, description, specs, wa_link, category, images)
            html_file.write_text(new_html, encoding='utf-8')
            print(f'  [OK]  {rel_path} — {len(images)} img, {len(specs)} specs')
            ok += 1
        except Exception as e:
            print(f'  [ERR] {rel_path}: {e}')
            errors.append(rel_path)

    print(f'\n✓ Completados: {ok}/{len(MINIMAL_PAGES)}')
    if errors:
        print(f'✗ Errores: {errors}')


if __name__ == '__main__':
    main()
